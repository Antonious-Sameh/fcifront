...
import { ExternalLink, BookOpen, ArrowLeft, ArrowRight, AlertCircle, Play, Clock } from 'lucide-react';
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-primary/10 text-primary">{course.platform}</span>
                  <span className="text-xs text-muted-foreground">{course.level}</span>
                </div>
                <h1 className="text-2xl font-black mb-1">{getName(course.title)}</h1>
                <p className="text-muted-foreground text-sm">{getName(course.description)}</p>
              </div>

              {/* حالة 1: فيديوهات داخلية */}
              {course.videos?.length > 0 ? (
                <div className={cn('flex gap-6', isAr ? 'flex-row-reverse' : 'flex-row')}>
                  {/* Player */}
                  <div className="flex-1 space-y-4">
                    {activeVideo && (
                      <>
                        ...
                        <div>
                          <h2 className="font-bold text-base">{activeVideo.title}</h2>
                  ...
                  <div className="w-72 shrink-0 space-y-2">
                    <p className="font-bold text-sm mb-3">
                      {isAr ? `قائمة التشغيل (${course.videos.length} فيديو)` : `Playlist (${course.videos.length} videos)`}
                    </p>
                    {course.videos.map((v, i) => (
                      <button key={v._id} onClick={() => setActiveVideo(v)}
                        ...
                        <span className={cn('w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center shrink-0',
                          activeVideo?._id === v._id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground')}>
                          {i + 1}
                        </span>
                ...
                <div className="rounded-2xl border border-border bg-card p-8 flex flex-col items-center gap-4 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {isAr ? `انتقل إلى ${course.platform} لمتابعة الكورس` : `Go to ${course.platform} to continue`}
                    </p>
                  </div>
                  {(course.courseUrl || course.url) ? (
                    ...
                    <p className="text-sm text-muted-foreground">{isAr ? 'رابط الكورس غير متاح حالياً' : 'Course link not available'}</p>
                  )}
                </div>
              )}
              ...
              {course.tags?.length > 0 && (
                <div className={cn('flex flex-wrap gap-2', isAr && 'flex-row-reverse')}>
                  {course.tags.map((tag, i) => (
                    <span key={i} className="text-xs px-3 py-1 rounded-full bg-muted text-muted-foreground border border-border">{tag}</span>
                  ))}
                </div>
              )}
            </div>
