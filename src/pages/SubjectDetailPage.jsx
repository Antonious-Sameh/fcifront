...
const LectureRow = ({ lecture, idx, isActive, onSelect, isAr, col }) => {
  const title = isAr
    ? lecture.title?.ar
    : lecture.title?.en || lecture.title?.ar;
  const hasVideo = !!(lecture.videoId || lecture.videoUrl);
  const hasPDF = !!lecture.pdfUrl;
  const vidId =
    extractVideoId(lecture.videoId) || extractVideoId(lecture.videoUrl);
  const thumb = vidId
        ...
        {thumb ? (
          ...
          <div
            className={cn(
              ...
              isActive ? col.bg : "bg-muted",
            )}
          >
            <span
              className={cn(
                "text-sm font-black",
                isActive ? col.text : "text-muted-foreground",
              )}
            >
              {idx + 1}
            </span>
      ...
      <div className="flex-1 min-w-0">
        <p
          className={cn(
            ...
            isActive ? col.text : "text-foreground",
          )}
        >
          {title}
        </p>
    ...
    >
      {lecture.pdfUrl ? (
        ...
        >
          ...
          <div
            className={cn(
              "p-3.5 rounded-2xl bg-background/80 shadow-sm shrink-0",
              col.text,
            )}
          >
          ...
          {/* Buttons */}
          <div
            className={cn("flex gap-2 shrink-0", isAr && "flex-row-reverse")}
          >
            <a
...
const SubjectDetailPage = () => {
  ...
  const lectures =
    subject?.lectures
      ?.filter((l) => l.type === "lecture" && l.isVisible !== false)
      .sort((a, b) => a.order - b.order) || [];
  const sections =
    subject?.lectures
      ?.filter((l) => l.type === "section" && l.isVisible !== false)
      .sort((a, b) => a.order - b.order) || [];
  ...
  const vidId =
    extractVideoId(active?.videoId) || extractVideoId(active?.videoUrl || "");

  return (
    <>
      <Helmet>
        <title>{getName(subject.name)} | Student Core</title>
      </Helmet>
      <DashboardLayout>
        <div
        ...
        >
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-5 transition-colors",
              isAr && "flex-row-reverse",
            )}
          >
            {isAr ? (
            ...
            {isAr
              ? TERM_META[term]?.ar || "الترم"
              : TERM_META[term]?.en || "Term"}
          </button>

          {/* Subject Header */}
          <div
            className={cn(
              "flex items-start gap-4 mb-7",
              isAr && "flex-row-reverse",
            )}
          >
          ...
          <div
            className={cn(
              "flex gap-1 mb-6 bg-muted/50 p-1 rounded-xl w-fit",
              isAr && "flex-row-reverse",
            )}
          >
            {[
              {
                key: "lectures",
                label: isAr ? `المحاضرات` : "Lectures",
                count: lectures.length,
                icon: Video,
              },
              {
                key: "sections",
                label: isAr ? `السكاشن` : "Sections",
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {active && (
                  ...
                  >
                    ...
                    <h2
                      className={cn(
                        "font-bold text-base flex-1 truncate leading-snug",
                        isAr && "text-right",
                      )}
                    >
                ...
                {vidId ? (
                  ...
                  <div
                    className={cn(
                      "aspect-video rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3",
                      col.border,
                      col.bg,
                    )}
                  >
                    <PlayCircle
                      className={cn("w-14 h-14 opacity-25", col.text)}
                    />
                ...
                {active && (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        handleSelect(
                          Math.min(currentList.length - 1, selectedIdx + 1),
                        )
                      }
              ...
              <div className="space-y-3">
                ...
                <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${((selectedIdx + 1) / currentList.length) * 100}%`,
                    }}
                  />
                </div>
